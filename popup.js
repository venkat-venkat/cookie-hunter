...
document.getElementById('open-popup1').onclick = openAnyCookieManager;
document.getElementById('open-currentpage-popup').onclick = openTopTabCookieManager;
document.getElementById('open-currentpage-popup').disabled = true;

chrome.runtime.sendMessage('getActiveTab', function(tab) {
    if (tab && tab.url && tab.url.startsWith('http')) {
        activeTab = tab;
        document.getElementById('open-currentpage-popup').disabled = false;
        document.getElementById('open-currentpage-popup').title =
            'View all cookies for ' + tab.url;
    } else {
        document.getElementById('open-currentpage-popup').remove();
    }
});
